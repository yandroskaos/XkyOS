...
PUBLIC dword GetThreadResult(HANDLE handle)
{
	if(!handle)
		return 0;
	return ((THREAD*)handle)->exit_code;
}

PUBLIC void WaitForThread(HANDLE handle)
{
...
//Scheduler
//FCFS
PRIVATE bool FCFSSchedulingPolicy(IN XID _xid, IN EXECUTION* _execution)
{
	if(last_time)
	{
		return false;
...
PRIVATE bool RoundRobinSchedulingPolicy(IN XID _xid, IN EXECUTION* _execution)
{
	if(last_time)
	{
		return false;
