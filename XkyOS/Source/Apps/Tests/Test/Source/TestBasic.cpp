...
PUBLIC void Main()
{
	...
	{
		//PCI
		string PCI_HEADER = STRING("Enumerating_PCI");
		...
		//Memory
		string MEM_HEADER = STRING("Asking_For_Memory");
		c.WriteLn(&MEM_HEADER, red);

		byte* pages = XKY_PAGE_Alloc(0x10000000, 2);
		if(pages)
		{
			c.WriteLn(&sok, blue);
		}
		...
		//Disk
		string DSK_HEADER = STRING("Asking_For_Disk");
		c.WriteLn(&DSK_HEADER, red);

		SECTOR disk_sector = XKY_DISK_Alloc(16384, 2);
		if(disk_sector)
		{
			c.WriteLn(&sok, blue);
		}
		...
		if(XKY_DISK_Read(disk_sector, pages, 2))
		{
			c.WriteLn(&sok, blue);
			...
			if(XKY_DISK_Write(disk_sector, pages, 2))
			{
				c.WriteLn(&sok, blue);
			}
			else
